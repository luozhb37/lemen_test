...
明细开票（收入）-新增发票（按对账单）
    ...
    #按对账单明细开票
    点击发票
    查询-新增发票（按对账单）    ${model_fi_debitcredits_debitcreditno}    ${BUSINESSNO-SUIT-SEAEXPORT}
    全选记录-列表页面
    点击开票-新增发票
    全选记录-进入详情框架
    ${write_invoiceno}    明细开票-按对账单
    关闭所有弹出框
...
明细开票-按账单（debit）-新增发票页面
    ...
    点击新增发票
    新增发票-点击按账单
    查询-新增发票（按对账单）    ${debitno}    EXP1910130
    全选记录-列表页面
    点击开票-新增发票
    全选记录-进入详情框架
    ${sign_invoiceno}    明细开票-按对账单
    关闭所有弹出框
...
对账并标记开票（收入）-新增对账单页面
    ...
    #查询对账单
    查询-对账单管理    fi_checkaccounts_checkaccountno=${model_fi_debitcredits_debitcreditno}
    ${content}    获取列表记录数
    应该包含    ${content}    共1条
    #查询发票
    点击发票
...
对账并登记发票（支出）-新增对账单页面
    ...
    点击发票
    点击发票管理
    查询-发票管理    write_invoiceno=${model_fi_debitcredits_invoiceno}
    ${content1}    获取列表记录数
    应该包含    ${content1}    共1条
    点击结算管理
    #费用列表显示对账单号、发票号
...
汇总开票-按对账单-对账单管理列表
    ...
    点击结算管理
    点击对账
    查询-对账单管理    qy_businessno=${BUSINESSNO-SUIT-SEAEXPORT}
    全选记录-列表页面
    点击开票-对账单管理列表
...
标记开票-按对账单-对账单管理列表
    ...
    点击结算管理
    点击对账
    查询-对账单管理    fi_checkaccounts_checkaccountno=${check_no_rec}
    全选记录-列表页面
    点击开票-对账单管理列表
